        ...
        try 
        {
        	for (Entity e : players) 
        	{
                if (e instanceof EntityPlayer && !e.getName().equalsIgnoreCase(mc.player.getName())) 
                {
                	...
                	{
                        knownPlayers.add(e);
                        Minecraft.getMinecraft().player.sendMessage(new TextComponentString(ChatFormatting.DARK_GRAY + "[BlmClient] " + ChatFormatting.WHITE + "<VisualRange> " + ChatFormatting.GOLD +  e.getName() + " entered visual range."));
                	}
                }
        	}
        } 
